#!/bin/bash
# Build Reservoir firmware and produce both output formats.
#   build/reservoir.bin         — app-only (for OTA via Headwaters)
#   build/reservoir_merged.bin  — merged  (for web flasher, full flash at 0x0)
#
...
echo "========================================"
echo "Building Reservoir ..."
echo "========================================"
idf.py build

# Create merged binary (for web flasher — flashable at 0x0)
esptool.py --chip esp32s3 merge_bin -o "$OUTPUT_DIR/reservoir_merged.bin" \
    ...
    0x10000 "$OUTPUT_DIR/reservoir.bin"
...
ls -lh "$OUTPUT_DIR/reservoir.bin"
echo ""
echo "Merged binary (for web flasher):"
ls -lh "$OUTPUT_DIR/reservoir_merged.bin"
echo ""
